...
const getAll = async(req, res) => {//вернем не все книги а тот который добавил пользователь 
   ...
   const result = await Comment.find({owner}, " -updatedAt",{skip,limit}) //если хотим отдавать какой то поля в массиве то передаем их в скобке на пример "title author name"б а ессли из не передавать то поставим передними минус как в примере "-title -author -name"
